...
export default function Quotation() {
  ...
  var assistences = [
    ...
    { id: 3, title: "Transporte", text: "Para levar o ciclista em caso de acidente ou mal súbito.", key: "transporte" },
  ];
  ...
  useEffect(() => {
    ...
    try {
      const selectBikePlan = (id, plan_id, plan_code) => {
        ...
        //if (!/^[0-9]{1,4}$/.test(id)) { return; }
        //if (!/^[0-9]{1,4}$/.test(plan_id)) { return; }

        //if (typeof plan_code !== "string" || plan_code.length < 3) { return; }
        //if (typeof plan_name !== "string" || plan_name.length < 3) { return; }
        var selected = true;
        var selectedPlan = null;
        ...
        //var selectedPlan = planos.find((plan) => (plan.id == id && plan.plan_id == plan_id));
        ...
        //if (selectedPlanCode) {
        //  selectedPlan = planos.find((plan) => (plan.plan_code == selectedPlanCode));
        //
        //  if (selectedPlan && selectedPlan.id) { 
        //    setSelectedButton(selectedPlan.id);
        //    setSelectedPlanCode(selectedPlan.plan_code);
      ...
      //
      //if (!selectedPlan && Array.isArray(planos) && planos.length > 0) {
      //  selectedPlan = planos.find((plan) => (plan.plan_code == selectedPlanCode || 'mobi_active'));
      //  
      //  if (selectedPlan && selectedPlan.id) {
      //    setSelectedButton(selectedPlan.id);
      //    setSelectedPlanCode(selectedPlan.plan_code);
        ...
        <div className=" mx-auto">
          ...
          <div className="flex flex-col md:flex-row flex-wrap justify-center self-start sm:gap-11 mt-5 animate__animated animate__fadeInRight">
            ...
            <div className="w-full md:max-w-[496px] flex flex-col-reverse sm:flex-row">
              <div className="inline-flex items-center justify-start w-auto mx-auto sm:w-1/2">
                <div className="">
                  <ul>
                    {protections.map((protection) => {
                      ...
                      if (protection.key === 'assistencia24' && plan_code === 'mobi_perform') sufix = 'ilimitada';
                  ...
                  <div className={`relative ${selectedPlanCode == "mobi_urban" ? "hidden" : ""}`}>
                    ...
                    <ul className="bg-[#EEE] w-fit mx-auto px-[15px] pb-[10px] pt-[13px] rounded-[10px] relative mt-2">
