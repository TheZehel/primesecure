class DataValidation extends Object {
    ...
    validarNome(nome){
        ...
        if (nome.length < 3){ return false; }// || !nome.includes(' ')){ return false; }
        return true;
    }
    validarEmail(email){
        email = email || '';
    ...
    validarTravelPayload(payload){
        ...
        if (!this.validarDestino(payload.destinyGroup)){ errorList.push('destinyGroup'); }
        if (!this.validarTotalPassageiros([payload.old0, payload.old1, payload.old2, payload.old3])){ errorList.push('ages'); }
        if (!this.validarDataTravel(payload.departure)){ errorList.push('departure'); }
        if (!this.validarDataTravel(payload.arrival)){ errorList.push('arrival'); }
        if (!this.validarNome(payload.name)){ errorList.push('full-name'); }
        if (!this.validarEmail(payload.email)){ errorList.push('email'); }
        if (!this.validarTelefone(payload.phone)){ errorList.push('phone'); }
