...
const PriceContainer = ({ submitForm }) => {
  ...
  const addPet = (details) => {
    ...
    if (editingIndex !== null) {
      ...
      try{ formPet = JSON.parse(formPet); }catch(error){  } //Evita erros caso formPet não seja um JSON válido
      ...
      let formPet = sessionStorage.getItem("formPetData");
      try{ formPet = JSON.parse(formPet); }catch(error){  }
  ...
  const removePet = (index) => {
    ...
    try{ formPet = JSON.parse(formPet); }catch(error){ formPet = null; }
  ...
  return (
    <div>
      ...
      {petAdded ? (
        <PetAdded
          pets={pets}
          nextStep={(step) => { submitForm(step, pets); }}
