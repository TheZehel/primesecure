...
export default function PetAdded({
  ...
  const handleContinue = () => {
    ...
    for (let i in pets) {
      ...
      if (!pet.plan.price) {
        errorArray.push("plan-price");
      }

      if (!pet.plan.title) {
        errorArray.push("plan-title");
      }

      if (!/^[0-9]{1,}$/.test(pet.plan.id)) {
        errorArray.push("plan-id");
      }
  ...
  return (
    <div>
      <div className="pb-6">
        <LayoutCotacaoPlanos
          title="Adicione Mais Pets ou Continue."
          position={0}
        />
      </div>
        ...
        return (
          ...
          >
            <div className="flex flex-col justify-center">
              <div
                className={`w-30 text-left text-xl font-bold mr-10 ${
                  error && error.length > 0 ? "text-alertRed" : "text-grayPrime"
                }`}
              >
                {pet.name}{" "}
                <span
                  className={`mt-auto text-green-600 text-base ${
                    index == 0 ? "hidden" : ""
                  }`}
                >
                  (-{index < 3 ? index * 10 : 30}%)
              ...
              <div className="flex items-center space-x-2">
                <div
                  className={`w-30 h-7 text-lg font-semibold ${
                    error && error.includes("plan-title")
                      ? "text-alertRed"
                      : "text-grayPrime"
                  }`}
                >
                ...
                <div
                  className={`w-30 h-7 text-center text-md font-bold flex ${
                    error && error.includes("plan-price")
                      ? "text-alertRed"
                      : "text-bluePrime"
                  }`}
                >
            ...
            <div className="block w-fit space-y-2 text-right">
              <div
                className="cursor-pointer font-semibold text-sm hover:text-bluePrime2"
                onClick={() => {
                  onEditPet(index);
                  window.dataLayer = window.dataLayer || [];
                  window.dataLayer.push({ event: "editar-pet" });
                }}
              >
                Editar
              </div>
              <div
                className="cursor-pointer font-semibold text-sm hover:text-alertRed"
                onClick={() => {
                  onRemovePet(index);
                  window.dataLayer = window.dataLayer || [];
                  window.dataLayer.push({ event: "remover-pet" });
                }}
              >
                Remover
              </div>
      ...
      <div
        className={`w-96 h-7 text-center text-grayPrime text-lg font-semibold mx-auto ${
          pets.length < 1 ? "hidden" : ""
        }`}
      >
        Ganhe{" "}
        <span className="text-bluePrime font-bold">
          {pets.length < 3 ? pets.length * 10 : 30}%
        </span>{" "}
        de desconto no segundo pet
      </div>
      <div className="flex items-center space-x-4 ">
        <div
          className="w-80 h-10 bg-cyan-500 hover:bg-bluePrime2 rounded-2xl shadow  mx-auto mt-5 text-white flex items-center justify-center space-x-6 cursor-pointer"
        ...
        >
          <span
            ...
            onClick={(e) => {
              window.dataLayer = window.dataLayer || [];
              window.dataLayer.push({ event: "adicionar-outro-pet" });
            }}
          >
            Adicionar Outro Pet
          </span>
      ...
      <div className="flex items-center space-x-4 ">
        ...
        >
          <span
            ...
            onClick={(e) => {
              window.dataLayer = window.dataLayer || [];
              window.dataLayer.push({ event: "pets-adicionados" });
            }}
          >
            Continuar Compra
          </span>
