...
function resolveAllLinks(...args) {
  return helper.resolveLinks
    ...
    .replace(
      /<a\s+href=("|')([^/]+\/\/[^'"]+)("|')([^>]*)/g,
      function ($0, quote, externalUrl, endQuote, attrs) {
        return !/\s+(rel|target)=/i.test(attrs)
          ? `<a href=${
              quote + externalUrl + endQuote
            } target="_blank" rel="noopener noreferrer"${attrs}`
          : $0;
      }
    );
}
  ...
  if (items.length) {
    ...
    items.forEach((item) => {
      ...
      if (!hasOwnProp.call(item, "longname")) {
        ...
        itemsNav += `<li>${kindAbbrTag} ${linktoFn(
          item.longname,
...
function buildNav(members) {
  ...
  // Adiciona Namespaces e seus Membros
  if (members.namespaces.length) {
    nav += "<h3>Namespaces</h3><ul>";
    members.namespaces.forEach((namespace) => {
      if (!seenNamespaces[namespace.longname]) {
        seenNamespaces[namespace.longname] = true;

        nav += `<li>${linkto(namespace.longname, namespace.name)}`;
        ...
        const namespaceMembers = [
          ...
          (member) =>
            member.memberof === namespace.longname && !seen[member.longname]
        );
        ...
        if (namespaceMembers.length) {
          ...
          namespaceMembers.forEach((member) => {
            ...
            nav += `<li>${linkto(member.longname, member.name)}</li>`;
          });
          nav += "</ul>";
        }
  ...
  if (members.globals.length) {
    const globalNav = members.globals
      .filter(({ longname }) => !seen[longname])
      .map(({ longname, name }) => {
        seen[longname] = true;
        return `<li><abbr title="Global">Glo</abbr> ${linkto(
          longname,
          name
        )}</li>`;
      })
